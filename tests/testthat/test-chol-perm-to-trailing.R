    ...
    for(seed in seeds) {
      ...
      for(i in 1:(p - 1)) {
        for(j in (i + 1):p) {
          ...
          expect_equal(got$l_ii, truth$l_ii,
            tolerance = 1e-10,
            info = sprintf(
              "l_ii mismatch p=%d seed=%d i=%d j=%d",
              p, seed, i, j
            )
          )
          expect_equal(got$l_ji, truth$l_ji,
            tolerance = 1e-10,
            info = sprintf(
              "l_ji mismatch p=%d seed=%d i=%d j=%d",
              p, seed, i, j
            )
          )
          expect_equal(got$l_jj, truth$l_jj,
            tolerance = 1e-10,
            info = sprintf(
              "l_jj mismatch p=%d seed=%d i=%d j=%d",
              p, seed, i, j
            )
          )
        }
