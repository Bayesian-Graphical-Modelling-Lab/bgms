...
for(delta in delta_grid) {
  ...
  for(i in seq_len(p)) {
    test_that(sprintf(
      "SBC q=10: K_ii[%d] ranks uniform under hierarchical (delta=%g)",
      i, delta
    ), {
      expect_gt(ks_p[i], 0.01,
        label = sprintf(
          "ks_p=%.3g (delta=%g, K_ii[%d])",
          ks_p[i], delta, i
        )
      )
    })
  ...
  test_that(sprintf(
    "SBC q=10: hierarchical-spec gamma marginal tracks p_inc (delta=%g)",
    delta
  ), {
    ...
    expect_lt(abs(mean_post_gamma - mean_true_gamma), 4 * se_gap,
      label = sprintf(
        "|post-mean(gamma) - true-mean(gamma)|=%.3g, SE=%.3g",
