...
# ==============================================================================
# Test 8: Marginal PL matches direct Gaussian-integral reference
# ==============================================================================
# After integrating y out of the joint
#   L(x, y) = main_x(x) + x' A_xx x + 2 x' A_xy y + b_y' y + y' A_yy y
# with A_yy = -Lambda/2, the marginal log-density is
#   marg_L(x) = main_x(x) + x' M x + 2 (A_xy mu_y)' x + const,
#   M         = A_xx + 2 A_xy Sigma A_xy',  mu_y = Sigma b_y.
#
# The marginal-PL rest score for discrete variable s is therefore
#   rest_s = 2 sum_{j != s} M[s, j] x_j + 2 (A_xy mu_y)_s,
# and the x_s = c exponent is main_x_s(c) + c^2 M[s, s] + c * rest_s.
#
# This test evaluates log p(x_s | x_{-s}) via the Gaussian integral and
# compares against the C++ log_pseudoposterior under the marginal-PL mode.
# It guards against a regression where the cross-induced term in M
...
test_that("marginal PL matches direct Gaussian integration of the joint", {
  ...
  mu_y <- c(0.2, -0.15)
  A_xy <- matrix(c(0.25, -0.1, 0.18, 0.35), nrow = p, ncol = q, byrow = TRUE)
  Lambda <- matrix(c(2.0, 0.3, 0.3, 1.4), nrow = q)
  R_chol <- chol(Lambda)
  ...
  res_marg <- mixed_test_logp_and_gradient(
    params, x_obs, y_obs, num_cats, is_ord, base_cat,
    edge_ind, "marginal", 2.5
  )
  res_cond <- mixed_test_logp_and_gradient(
    params, x_obs, y_obs, num_cats, is_ord, base_cat,
    edge_ind, "conditional", 2.5
  )

  # All non-PL terms (GGM conditional, priors, Cholesky Jacobian) are
  # identical across the two modes, so the mode-to-mode difference
  # isolates the PL contribution.
  expect_equal(
    res_marg$value - res_cond$value,
    direct_marginal_pl() - direct_conditional_pl(),
    tolerance = 1e-10
  )
})
