...
| ~~11~~ | ~~Document `bgmCompare` free functions~~ | ~~src/bgmCompare/~~ | ~~Done. Added `@file` blocks to all three headers. Added `/** */` with `@param`/`@return` to 16 free functions across `bgmCompare_helper.h` (7 functions), `bgmCompare_logp_and_grad.h` (8 functions), and `bgmCompare_sampler.h` (1 function). Used `@see gradient()` cross-references to avoid repeating shared parameter docs.~~ |
| ~~12~~ | ~~Add pkgdown reference sections~~ | ~~_pkgdown.yml~~ | ~~Done. Added grouped `reference:` sections (Model fitting, Posterior methods, Simulation and prediction, Extractors, Legacy, Datasets, Package) covering all 19 Rd pages. Added `articles:` section for vignettes.~~ |
| ~~13~~ | ~~Document `base_model.h`~~ | ~~src/models/~~ | ~~Done. Added class-level `/** */` explaining the virtual interface, subclass hierarchy (GGM/OMRF), and method groups. Added Doxygen blocks to all ~30 virtual methods organized by section (capability queries, sampling, edge selection, vectorization, infrastructure, adaptation, missing data, edge prior).~~ |
...
| ~~18~~ | ~~Update intro vignette to mention GGM~~ | ~~vignettes/intro.Rmd~~ | ~~Done. Rewrote introduction to list three variable types (ordinal, Blume-Capel, continuous). Added GGM section with `variable_type = \"continuous\"` example and note about `na_action = \"impute\"`.~~ |
| ~~19~~ | ~~Update NEWS.md for GGM imputation~~ | ~~NEWS.md~~ | ~~Done. Added \"New features\" section to v0.1.6.4 with entries for GGM support (`variable_type = \"continuous\"`) and missing-data imputation (`na_action = \"impute\"`).~~ |
| ~~20~~ | ~~Update README for GGM~~ | ~~Readme.Rmd~~ | ~~Done. Updated tagline to \"Bayesian analysis of graphical models\" (dropped \"binary and ordinal\"). Intro paragraph now mentions continuous variables and GGM. Main functions section notes `variable_type = \"continuous\"` for GGM.~~ |
| ~~21~~ | ~~Add `.lintr` config~~ | ~~.lintr~~ | ~~Done. Created `.lintr` with `linters_with_defaults()`. Disabled linters that conflict with bgms style (`assignment`, `spaces_left_parentheses`, `object_name`, `return`, etc.). Enabled `T_and_F_symbol_linter()`, `seq_linter()`, `line_length_linter(100)`. Excludes `R/RcppExports.R` and `inst/`. 64 line-length lints remain as baseline.~~ |
