...
| 21 | Add `.lintr` config | .lintr | Add a minimal lintr configuration with `linters_with_defaults()`. Disable the assignment linter (handled by `bgms_style`). Enable `T_and_F_symbol_linter()`, `seq_linter()`, `line_length_linter(100)`. Exclude `R/RcppExports.R`. |
