 ...
 *
 * Uses fixed-length leapfrog integration with full warmup adaptation:
 * - Heuristic initial step size
 * - Diagonal mass matrix estimation (windowed)
 * - Dual averaging step size adaptation
 *
 * Warmup schedule mirrors NUTSSampler (3 stages).
 */
...
public:
    ...
    {
        build_warmup_schedule(config.no_warmup);
    }
